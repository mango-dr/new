import { InMemoryDbService } from 'angular-in-memory-web-api';
import { Coffee } from './coffee';
import { Injectable } from '@angular/core';

@Injectable({
  providedIn: 'root',
})
export class InMemoryDataService implements InMemoryDbService {
  createDb() {
    const drinks = [
      { id: 1,
        imagePath: '/images/black-coffee.jpg',
        name: 'Black coffee',
        description: 'Simply put, black coffee is a combination of water and coffee without any milk. You should serve it without any added flavors such as honey, cream, and milk. The addition of these products changes the aroma and color of the coffee, turning it lighter-brown or white.',  
        price: 15
        },

      { id: 2,
        imagePath: '/images/latte.jpeg',
        name: 'Latte',
        description: 'Latte is a coffee-based drink made primarily from espresso and steamed milk. It consists of one-third espresso, two-thirds heated milk and about 1cm of foam.',
        price: 15
      },

      { id: 3,
        imagePath: '/images/cappuccino.png',
        name: 'Cappuccino',
        description: 'Cappuccino is espresso coffee topped with frothed hot milk or cream and often flavored with cinnamon.',
        price: 15
      },

      { id: 4,
        imagePath: '/images/americano.jpg',
        name: 'Americano',
        description: 'An Americano is a great option for those who are not fans of making their coffee with milk and need something a little stronger to help give them that extra caffeine.',
        price: 15
      },

      { id: 5,
        imagePath: '/images/espresso.jpg',
        name: 'Espresso',
        description: 'A strong coffee brewed by forcing hot water under pressure through darkly roasted, finely ground coffee beans.',
        price: 15
      },

      { id: 6,
        imagePath: '/images/ristretto2.jpg',
        name: 'Ristretto',
        description: 'This involves brewing 30mls of coffee over the same pour time as a normal espresso shot. A traditional espresso uses approximately 14 grams of coffee for a double espresso.',
        price: 15
      },

      { id: 7,
        imagePath: '/images/macchiato2.jpg',
        name: 'Macchiato',
        description: 'For a Hot Macchiato, a rich and bold espresso beverage, we add a layer of steamed milk, topped with two shots of espresso, then finished with milk foam.',
        price: 15
      },

      { id: 8,
        imagePath: '/images/flat_white.jpg',
        name: 'Flat White',
        description: 'The base of our flat white is a double shot of espresso combined with lightly aerated milk to create a velvety smooth espresso beverage.',
        price: 15
      },

      { id: 9,
        imagePath: '/images/iced_coffee2.png', 
        name: 'Iced Coffee',
        description: 'Great cold brew coffee will have a smoothness that is unlike any traditionally brewed coffee. Some might say that a bitter flavor is the goal, but done properly, cold brewing maximizes smoothness and minimizes bitterness.',
        price: 15
      },

      { id: 10,
        imagePath: '/images/Hot-Chocolate2.jpg',
        name: 'Hot Chocolate',
        description: 'Hot chocolate, also known as hot cocoa or drinking chocolate, is a heated drink consisting of shaved chocolate, melted chocolate or cocoa powder, heated milk or water, and usually a sweetener.',
        price: 15
      }
    ];
    return {drinks};
  }

  // genId(drinks: Coffee[]): number {
  //   return drinks.length > 0 ? Math.max(...drinks.map(coffee => coffee.id)) + 1 : 11;
  // }
}
